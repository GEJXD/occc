...
type binary_operator = Add | Subtract | Multiply | Divide | Mod [@@deriving show]
...
type exp = 
  ...
  | Binary of binary_operator * exp * exp [@@deriving show]

type statement = Return of exp [@@deriving show]
type function_definition = Function of { name : string; body : statement } [@@deriving show]

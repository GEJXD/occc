...
(* map mothods: add lookup find 
  note that functional data structure are persistent. *)
module StringMap = Map.Make (String)

(* a state variant to represent the current state,
  for more functional, I prefer do not use ref keywords *)
...
let replace_operand state = function
  | Pseudo id -> begin
      ...
      (* already have assigned this identifier, 
         return Assembly Stack operand *)
      ...
      | None -> 
          ...
          let new_state = {current_offset = new_offset; offset_map = new_map} in
...
let replace_pseudo_in_instruction state = function
  ...
  | Ret -> (state, Ret)
  | AllocateStack _ ->
      failwith "Internal error: AllocateStack shouldn't be present at this point"
...
let replace_pseudo_in_function (Function {name; instructions}) = 
  ...
  in (Function {name; instructions = fixed_instructions},
      final_state.current_offset )

let replace_pseudos (Program func_def) = 
  let fixed_def, last_stack_cell = 
    replace_pseudo_in_function func_def in

...
(* apply a typecheck function to an option variable *)
let opt_typecheck typecheck_fn = function
  | Some ast_node -> typecheck_fn ast_node
  | None -> ()

let rec typecheck_exp = function
  (* For function call, we need to ensure that the number of params are same
     with argument, and it is not a variable *)
...
let rec typecheck_block (Block blk) = List.iter typecheck_block_item blk
...
and typecheck_statement = function
  ...
  | Compound blk -> typecheck_block blk
...
and typecheck_local_var_decl { name; init; storage_class } =
  ...
  | Some Static ->
      ...
      in
      Symbols.add_static_var name ~t:Int ~init:vinit ~global:false
  | None ->
